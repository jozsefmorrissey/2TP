function domAop() {
  ...
  /**
   *  cutoPoint Object {event1: { cutpoint, function, group }
                        event2: { cutpoint, function, group }
                        ... }
   */
  ...
  function reset(groups) {
    ...
    for (let keyIndex = 0; keyIndex < eventKeys.length; keyIndex += 1) {
      ...
      for (let index = cpArr.length - 1; index > -1; index -= 1) {
        const eventObj = cpArr[index];
        let userSpecified = groups !== undefined && -1 < groups.indexOf(eventObj.group);
        if (eventObj.group === undefined || userSpecified) {
          const croppedArr = cpArr.slice(0, index)
          if (index < cpArr.length) {
              croppedArr.concat(cpArr.slice(index + 1));
          }
